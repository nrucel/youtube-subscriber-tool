...
class Anasayfa extends NR_Controller
{
	...
	public function index() {
		...
		if(isset($_GET['r']) and $_GET['r']){
			setcookie("referans", $_GET['r'], time() + (60*60*24*30));
		}
	...
	function giriss() {
		...
		if(!$this->session->userdata("user_code"))
		{
	        ...
	        $yt = json_decode($this->Nrmod->post("getCode",$data),true);

	        $this->session->set_userdata("user_code", $yt["data"]["user_code"]);
			$this->session->set_userdata("code", $yt["data"]["device_code"]);
		}

        $data["code"] = $this->session->userdata("user_code");
        
		
		$this->load->view("youtube/giris2.php",$data);
	}
	...
	function kontrol() {
		...
		if($this->session->userdata("rekey"))
    	{
			...
			$this->load->view("youtube/kontrol.php");
		}
		else
		{
